...
class RGBXTransformer(nn.Module):
    ...
    ##
    def init_weights_onepath(self, pretrained=None):
        if isinstance(pretrained, str):
            load_onepath_model(self, pretrained)
        else:
            raise TypeError('pretrained must be a str or None')

    ##
    def init_weights_onepath2(self, pretrained=None):
        if isinstance(pretrained, str):
            load_onepath2_model(self, pretrained)
        else:
            raise TypeError('pretrained must be a str or None')
...
##########
def load_onepath_model(model, model_file):
    # ONLY RGB
    # load raw state_dict
    t_start = time.time()
    if isinstance(model_file, str):
...
def load_onepath2_model(model, model_file):
    # ONLY NP++
    # load raw state_dict
    t_start = time.time()
    if isinstance(model_file, str):
...
class mit_b1_avn(RGBXTransformer):
    def __init__(self, fuse_cfg=None, stride0=4, **kwargs):
        super(mit_b1_avn, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], in_chans=2, num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, stride0=stride0)